...
class GRUTheano:
    ...
    def __init__(self, word_dim, hidden_dim=100, bptt_truncate=-1):
        ...
        c = np.zeros(3)
        U_decode = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (12, hidden_dim, hidden_dim))
        W_decode = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (12, hidden_dim, hidden_dim))
        b_decode = np.zeros((12, hidden_dim))
    ...
    def __theano_build__(self):
        ...
        def forward_prop_step_decode(x_t, y_t, s_t1_prev_d, s_t2_prev_d, s_t3_prev_d, c_t1_prev_d, c_t2_prev_d, c_t3_prev_d):
            # This is how we calculated the hidden state in a simple RNN. No longer!
            # s_t = T.tanh(U[:,x_t] + W.dot(s_t1_prev))
            ...
            # LSTM Layer 2
            i_t2_d = T.nnet.hard_sigmoid(U_decode[4].dot(s_t1_d) + W_decode[4].dot(s_t2_prev_d) + b_decode[4])
            f_t2_d = T.nnet.hard_sigmoid(U_decode[5].dot(s_t1_d) + W_decode[5].dot(s_t2_prev_d) + b_decode[5])
            o_t2_d = T.nnet.hard_sigmoid(U_decode[6].dot(s_t1_d) + W_decode[6].dot(s_t2_prev_d) + b_decode[6])
            g_t2_d = T.tanh(U_decode[7].dot(s_t1_d) + W_decode[7].dot(s_t2_prev_d) + b_decode[7])
            c_t2_d = c_t2_prev_d * f_t2_d + g_t2_d * i_t2_d
            s_t2_d = T.tanh(c_t2_d) * o_t2_d

            # LSTM Layer 3
            i_t3_d = T.nnet.hard_sigmoid(U_decode[8].dot(s_t2_d) + W_decode[8].dot(s_t3_prev_d) + b_decode[8])
            f_t3_d = T.nnet.hard_sigmoid(U_decode[9].dot(s_t2_d) + W_decode[9].dot(s_t3_prev_d) + b_decode[9])
            o_t3_d = T.nnet.hard_sigmoid(U_decode[10].dot(s_t2_d) + W_decode[10].dot(s_t3_prev_d) + b_decode[10])
            g_t3_d = T.tanh(U_decode[11].dot(s_t2_d) + W_decode[11].dot(s_t3_prev_d) + b_decode[11])
        ...
        def forward_prop_step_decode_test(x_t, o_t_pre_test, s_t1_prev_d_test, s_t2_prev_d_test, s_t3_prev_d_test, c_t1_prev_d_test, c_t2_prev_d_test, c_t3_prev_d_test):
            ...
            # LSTM Layer 2
            i_t2_d_test = T.nnet.hard_sigmoid(U_decode[4].dot(s_t1_d_test) + W_decode[4].dot(s_t2_prev_d_test) + b_decode[4])
            f_t2_d_test = T.nnet.hard_sigmoid(U_decode[5].dot(s_t1_d_test) + W_decode[5].dot(s_t2_prev_d_test) + b_decode[5])
            o_t2_d_test = T.nnet.hard_sigmoid(U_decode[6].dot(s_t1_d_test) + W_decode[6].dot(s_t2_prev_d_test) + b_decode[6])
            g_t2_d_test = T.tanh(U_decode[7].dot(s_t1_d_test) + W_decode[7].dot(s_t2_prev_d_test) + b_decode[7])
            c_t2_d_test = c_t2_prev_d_test * f_t2_d_test + g_t2_d_test * i_t2_d_test
            s_t2_d_test = T.tanh(c_t2_d_test) * o_t2_d_test

            # LSTM Layer 3
            i_t3_d_test = T.nnet.hard_sigmoid(U_decode[8].dot(s_t2_d_test) + W_decode[8].dot(s_t3_prev_d_test) + b_decode[8])
            f_t3_d_test = T.nnet.hard_sigmoid(U_decode[9].dot(s_t2_d_test) + W_decode[9].dot(s_t3_prev_d_test) + b_decode[9])
            o_t3_d_test = T.nnet.hard_sigmoid(U_decode[10].dot(s_t2_d_test) + W_decode[10].dot(s_t3_prev_d_test) + b_decode[10])
            g_t3_d_test = T.tanh(U_decode[11].dot(s_t2_d_test) + W_decode[11].dot(s_t3_prev_d_test) + b_decode[11])
