...
class GRUTheano:
    ...
    def __init__(self, word_dim, hidden_dim=100, bptt_truncate=-1):
        ...
        U_decode = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim),
                                     (4, hidden_dim * 2, hidden_dim))
        W_decode = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim),
                                     (4, hidden_dim * 2, hidden_dim * 2))
        va = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (hidden_dim * 2, 1))
        WA = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (2, hidden_dim * 2, hidden_dim * 2))
        UA = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (4, hidden_dim * 2, hidden_dim * 2))
    ...
    def __theano_build__(self):
        E, Ey_encode, Ey_decode, V, U,UA, W,WA, va, b, c, U_decode, W_decode, b_decode = self.E, self.Ey_encode, self.Ey_decode, self.V, self.U,self.UA, self.W, self.WA,self.va,self.b, self.c, self.U_decode, self.W_decode, self.b_decode
        ...
        def forward_prop_step_decode(x_t, y_t, s_t1_prev_d, c_t1_prev_d, M_t1, He_1):
            ...
            a_t1 = T.tanh(s_t1_prev_d.dot(WA[1]) + M_t1).dot(va)  # (len,1)
        ...
        def forward_prop_step_decode_test(x_t, o_t_pre_test, s_t1_prev_d_test, c_t1_prev_d_test, M_t1, He_1):
            ...
            a_t1 = T.tanh(s_t1_prev_d_test.dot(WA[1]) + M_t1).dot(va)  # (len,1)
            a_t1 = T.nnet.softmax(a_t1.T)[0]
            r_t1 = a_t1.dot(He_1)
        ...
        [o, s_t1_d, c_t1_d], updates = theano.scan(
            ...
            truncate_gradient=self.bptt_truncate,
            outputs_info=[None,
                          dict(initial=s_t1_encode[-1]),
                          dict(initial=c_t1_encode[-1])],
        ...
        self.bptt = theano.function([x, y], [dE, dU, dW, dWA, dUA, db, dV, dva, dc, dU_decode, dW_decode, db_decode])
...
def load_model_parameters_theano(path, modelClass=GRUTheano):
    ...
    E, U, W,  UA, WA,V,va, b, c, U_decode, W_decode, b_decode = npzfile["E"], npzfile["U"], npzfile["W"], npzfile["UA"], npzfile["WA"],npzfile["V"],npzfile["va"], npzfile["b"], npzfile["c"], npzfile["U_decode"], npzfile["W_decode"], npzfile["b_decode"]
