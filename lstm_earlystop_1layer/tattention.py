...
class GRUTheano:
    ...
    def __theano_build__(self):
        ...
        def forward_prop_step_decode(x_t, y_t, s_t1_matrix, s_t1_prev_d, c_t1_prev_d):
            # This is how we calculated the hidden state in a simple RNN. No longer!
            # s_t = T.tanh(U[:,x_t] + W.dot(s_t1_prev))
        ...
        def forward_prop_step_decode_test(x_t, s_t1_matrix, o_t_pre_test,s_t1_prev_d_test, c_t1_prev_d_test):
            # This is how we calculated the hidden state in a simple RNN. No longer!
            # s_t = T.tanh(U[:,x_t] + W.dot(s_t1_prev))
        ...
        [s_t1, c_t1], updates = theano.scan(
            ...
            outputs_info=[dict(initial=T.zeros(self.hidden_dim)),
                          dict(initial=T.zeros(self.hidden_dim))])
        s_t1_encode = s_t1 + s_t1_b
        c_t1_encode = c_t1 + c_t1_b
